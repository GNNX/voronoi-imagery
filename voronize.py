...
from skimage.filters import sobel, gaussian as gaussian_filter
...
def poisson_disc(img, n, k=30):
    ...
    def gen_rand_point_around(point):
        #print("gen_rand_point_around:", point)
    ...
    def has_neighbours(point):
        ...
        for dist, idx in zip(distances, idxs):
            ...
            f = tuple(tree.data[idx])
            #print(f)
            if dist < point_dist and dist < dists[int(f[0]), int(f[1])]:
                return True
